...
function Search(query, sort_type, categories) {
  this.query = query
  this.sort_type = typeof sort_type !== 'undefined' ? sort_type : sort.RELEVANCE;
...
Search.prototype._parseSearchPage = function(html) {
  ...
  rows.each(function(index, row) {
    ...
    try {
      var url = config.base_url + $('.detLink', row).attr('href');
      var result = {
        'title': title,
