...
var categores = require('./categories');
...
function Search(search_terms) {
  ...
  this.categories = typeof search_terms.categories !== 'undefined' ? categores[search_terms.categories] : '0';
}
...
Search.prototype._parseSearchPage = function(html) {
  ...
  rows.each(function(index, row) {
    ...
    try {
      ...
      var desc = $('.detDesc', row).html();
      var size = /Size\s([0-9\.]+)&.+;([a-zA-Z]+),/.exec(desc);
      size = size[1] && size[2] ? bytes.sizeToBytes(size[1], size[2]) : null;
      var result = {
        'title': title,
