...
Torrent.find_by_id = function(id, callback) {
  // TODO: Move BASE_URL out to somwhere configurable
  var url = "http://thepiratebay.se" + "/torrent/" + encodeURIComponent(id)
  var _this = this;
...
Torrent.parse_torrent_page = function(html) {
  ...
  var results = {
    ...
    "size": $("dd", row).eq(2).text(), //REGEX: \((\d+).+\)
    "date": $(".col2 dd", row).eq(0).text(), // Parse Time
